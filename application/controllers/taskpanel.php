...
class Taskpanel extends CI_Controller {
				...
				if($this->session->userdata('user_id')){
					if($this->input->post('submit',true)){
						if($this->input->post('task_name',true) &&
							$this->input->post('task_date',true)){
	...
	public function addtask() {
						...
						$store_info=$this->category->getstorebyid($store_id);
						$store_name=$store_info->s_store_name;
						 
				if($this->session->userdata('user_id')){
					 	 	$_POST['task_name']=$store_name;
							$_POST['task_date']=date('Y-m-d h:m:s');
							$response=$this->task_panel->addtaskpanel();
							$this->session->set_flashdata('result', 'Task Added successfully');
